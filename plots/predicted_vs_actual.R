...
p8 <- ggplot(log_pr, aes(Predicted, Actual, label=Team, fill=Team, color=Team, group=1)) + geom_abline(slope=1, color='gray',group=2) + geom_point() + geom_text(nudge_y=3,size=5) + coord_cartesian(xlim=c(0,82), ylim=c(0,82)) + scale_fill_manual(values=as.character(colors)) + scale_color_manual(values=c[order(t)])+ theme(legend.position="none") + ggtitle('Logisitic Regression Probability') + theme(plot.margin=unit(c(1,0,0,1),"cm"),axis.title=element_text(size=22),axis.text=element_text(size=16),plot.title = element_text(size=26)) + xlab('Predicted Wins') + ylab('Actual Wins')
